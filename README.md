...
- **덱 빌더** — 별도 페이지로 구현 예정
  - 카드 목록에서 선택해 50장 덱 구성
  - 미보유 카드 흐리게 표시 (추가는 가능)
  - 50장 초과 시 추가 불가 (하드 제한), 미만 시 저장 불가
...
- **2026-06-26** — PR 카드 5종 추가, PR 덱 탭 및 레어리티 필터 추가, 여분 수량 표시 개선, 트리거 아이콘 13종 전체 매핑 추가, 트리거 언어별 텍스트 표시 추가, CX 카드 트리거 데이터 수정, 전체 카드 `parallel` 필드 추가, 유루캠 로고 파비콘 및 헤더 아이콘 적용
